...
TitanicT %>% 
  ...
  prop_test(Survived ~ NULL,p=1/3,success ="1")
TitanicT %>% 
  mutate(Survived=as.factor(Survived)) %>% 
  prop_test(Survived ~ NULL,p=1/3,success ="1",z=TRUE)
```
...
TitanicT %>%  
  prop_test(Sex ~ NULL,p=.5, order = c("female","male"))
TitanicT %>%  
  prop_test(Sex ~ NULL,p=.5, success =  "male", z=TRUE)
```
